...
public class LiftAutoCmd extends CommandBase{
    ...
    public void execute() {
        ...
        if (distToSetPoint!=0 && Math.abs(distToSetPoint) < 0.05) {
            liftSubsystem.setLiftMotor((-MathMethods.signDouble(distToSetPoint)*0.25));
        }
        if (Math.abs(distToSetPoint)>=0.05 && Math.abs(distToSetPoint)<=0.10) {
            liftSubsystem.setLiftMotor((-5*distToSetPoint));
        }
        if (Math.abs(distToSetPoint) > 0.10) {
            liftSubsystem.setLiftMotor(-MathMethods.signDouble(distToSetPoint)*0.5);
        }

        if (Math.abs(distToSetPoint) < 0.0008) {
